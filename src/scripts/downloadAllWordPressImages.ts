...
class WordPressImageDownloader {
  ...
  async downloadAllWordPressImages(): Promise<void> {
    try {
      console.log('🖼️  Starting WordPress image download process...');
      ...
      const productsWithWordPressImages = products.filter(p => 
        p.image && p.image.includes('forzabuilt.com')
      );
      
      console.log(`📊 Found ${productsWithWordPressImages.length} products with WordPress images`);
      
      if (productsWithWordPressImages.length === 0) {
      ...
      for (const product of productsWithWordPressImages) {
      ...
      console.log('\n🎉 WordPress image download completed!');
      ...
      console.error('❌ Error in WordPress image download process:', error);
    } finally {
...
if (require.main === module) {
  const downloader = new WordPressImageDownloader();
  downloader.downloadAllWordPressImages();
}

export { WordPressImageDownloader };
