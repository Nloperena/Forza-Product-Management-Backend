...
class LocalImageUploaderToVercelBlob {
  private pool: Pool;
  ...
  async uploadAllImages(): Promise<void> {
    try {
      console.log('🚀 Starting local image upload to Vercel Blob...');
      ...
      for (const product of products) {
        try {
          if (!product.image || product.image === '/placeholder-product.svg' || product.image.startsWith('https://')) {
            console.log(`⏭️  Skipping product ${product.name} - no image, placeholder, or already has Vercel URL`);
            continue;
          }
          ...
          if (uploadedCount % 10 === 0) {
            console.log(`📊 Progress: ${uploadedCount} images uploaded...`);
          }
...
if (require.main === module) {
  const uploader = new LocalImageUploaderToVercelBlob();
  uploader.uploadAllImages()
    .then(() => {
      console.log('✅ Local image upload script completed successfully');
      process.exit(0);
    })
    .catch((error) => {
      console.error('❌ Local image upload script failed:', error);
      process.exit(1);
    });
}

export { LocalImageUploaderToVercelBlob };
