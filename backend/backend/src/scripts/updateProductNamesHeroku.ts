...
class ProductNameUpdater {
  ...
  async execute(): Promise<void> {
    ...
    console.log('🔄 Updating product names on Heroku...');
    ...
    if (!isPostgres) {
      ...
      console.warn('⚠️  Please set DATABASE_URL to your Heroku database URL.\n');
    }
...
async function main() {
  ...
  if (!process.env.DATABASE_URL && !process.env.POSTGRES_URL) {
    ...
    console.error('   Example: $env:DATABASE_URL="postgres://..." npx ts-node src/scripts/updateProductNamesHeroku.ts --live');
    process.exit(1);
  }
